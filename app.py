...
def build_city_summary(df2_raw, df2_clean, df):
    ...
    nc_data = df2_clean[df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)]
    nc_total = nc_data.groupby("City").size().reset_index(name="Total Not Connected")

    conn_data = df2_clean[~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)]
    conn_total = conn_data.groupby("City").size().reset_index(name="Total Connected")

    pos_data = df2_clean[df2_clean["Sub Disposition"].isin(POSITIVE_LIST)]
    pos_total = pos_data.groupby("City").size().reset_index(name="Positive Feedback")

    neg_data = df2_clean[df2_clean["Sub Disposition"].isin(NEGATIVE_LIST)]
    neg_total = neg_data.groupby("City").size().reset_index(name="Negative Feedback")

    gen_data = df2_clean[
        (~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)) &
        (~df2_clean["Sub Disposition"].isin(EXCLUDED_LIST))
    ]
...
def build_applicator_summary(df2_raw, df2_clean, df):
    ...
    nc = df2_clean[df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)].groupby("Applicator").size().reset_index(name="Not Connected")
    conn = df2_clean[~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)].groupby("Applicator").size().reset_index(name="Connected Calls")
    pos = df2_clean[df2_clean["Sub Disposition"].isin(POSITIVE_LIST)].groupby("Applicator").size().reset_index(name="Positive Feedback")
    neg = df2_clean[df2_clean["Sub Disposition"].isin(NEGATIVE_LIST)].groupby("Applicator").size().reset_index(name="Negative Feedback")
    gen = df2_clean[
        (~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)) &
        (~df2_clean["Sub Disposition"].isin(EXCLUDED_LIST))
    ...
    grand_total = {
        ...
        "Not Connected": df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST).sum(),
        "Connected Calls": (~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)).sum(),
        "Positive Feedback": df2_clean["Sub Disposition"].isin(POSITIVE_LIST).sum(),
        "Negative Feedback": df2_clean["Sub Disposition"].isin(NEGATIVE_LIST).sum(),
        "General Feedback": ((~df2_clean["Sub Disposition"].isin(NOT_CONNECTED_LIST)) & (~df2_clean["Sub Disposition"].isin(EXCLUDED_LIST))).sum(),
        "NPS Given": (df2_clean["NPS SCORE"] > 0).sum(),
...
def build_connectivity_breakdown(df2_clean):
    ...
    pos = df2_clean[df2_clean["Sub Disposition"].isin(POSITIVE_LIST)]
    neg = df2_clean[df2_clean["Sub Disposition"].isin(NEGATIVE_LIST)]
    gen = df2_clean[
